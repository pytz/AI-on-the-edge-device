...
bool ClassFlowPostProcessing::ReadParameter(FILE* pfile, string& aktparamgraph)
{
    ...
    while (this->getNextLine(pfile, &aktparamgraph) && !this->isNewParagraph(aktparamgraph))
    {
        ...
        if ((toUpper(_param) == "ALLOWNEGATIVERATESONOVERFLOW") && (zerlegt.size() > 1))
        {
            if (toUpper(zerlegt[1]) == "TRUE")
                for (_n = 0; _n < NUMBERS.size(); ++_n)
...
//        NUMBERS[j]->lastvalue = imagetime;    // must only be set in case of good value !!! --> move to the end
        ...
        #ifdef SERIAL_DEBUG
            ESP_LOGD(TAG, "Overflow detection: maximumValue= %f, overflowDistance=%f, preValue=%f, acceptOverflow=%d", maximumValue,
            overflowDistance, distanceNegative, acceptOverflow);
        #endif
        }
